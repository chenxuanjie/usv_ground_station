...
- `X` 表示艇端已经完成了解析、检查和实际切换处理后的结果，不是“收到即回”。
- 只要原始报文能够正确解析出 `seq`，艇端就应尽量返回一条 `X` 报文。
- 若报文损坏到连 `seq` 都无法解析，则无法构造有效应答，此时仅做本地日志记录。

### `seq` 与 `ack_seq` 的匹配逻辑

- `C.seq` 表示控制命令自己的流水号，由命令发送端生成。
- `X.seq` 表示应答报文自己的流水号，由艇端生成。
- `X.ack_seq` 表示这条应答对应的是哪一条控制命令，其值应等于原始 `C` 报文中的 `seq`。

匹配规则如下：

- 地面站发送 `C` 报文时，先为该命令分配一个唯一的 `seq`。
- 艇端处理完成后返回 `X` 报文，并将被应答命令的 `seq` 填入 `ack_seq`。
- 地面站收到 `X` 报文后，通过 `ack_seq` 去匹配原始 `C.seq`，从而确认这条应答对应哪一条控制命令。

注意：

- 当前版本中，命令与应答的关联是通过 `ack_seq -> C.seq` 完成的，不是通过 `X.seq` 去匹配。
