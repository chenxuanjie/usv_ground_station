    ...
    return (
      ...
      <div className="flex items-center gap-3 pointer-events-auto">
        <button
          ...
          className={ui?.statusBar?.iconBtn || "w-10 h-10 flex items-center justify-center border border-cyan-500/50 bg-cyan-900/20 rounded hover:bg-cyan-500/20 active:scale-95 transition-all"}
        >
          <Menu className={`${ui?.statusBar?.icon || 'text-cyan-400'} w-5 h-5`} />
        </button>
        <div>
          <h1 className={ui?.statusBar?.title || "font-mono font-bold text-lg text-cyan-50 tracking-wider flex items-center gap-2 drop-shadow-[0_0_5px_rgba(34,211,238,0.8)]"}>
            {title}
          </h1>
          <div className={`flex items-center gap-3 ${ui?.statusBar?.meta || 'text-[10px] font-mono'}`}>
