...
function BoatGroundStation() {
    ...
    useEffect(() => {
        const connectToBridge = () => {
            ...
            ws.onclose = () => {
                ...
                // 这里使用 useRef 里的 lang 可能会很麻烦，简化起见，Websocket 断开日志直接用当前语言或硬编码
                // 为了简单，我们让它根据当前渲染周期的 t 函数 (虽然在闭包里可能不即时更新，但够用了)
                addLog('SYS', translations['zh'].log_ws_disconnect); // System log usually static
            };
            ...
            ws.onmessage = (event) => {
                ...
                if (msg.startsWith('CURRENT_CONFIG')) {
                    ...
                    if (status === 'ONLINE') {
                        setTcpStatus('ONLINE');
                        // 动态获取当前语言的日志
                        setLogs(prev => [{id: Date.now(), time: new Date().toLocaleTimeString(), dir: 'SYS', msg: translations[lang === 'zh' ? 'zh' : 'en'].log_tcp_online}, ...prev].slice(0, 50));
                    } else if (status === 'OFFLINE') {
                        setTcpStatus('OFFLINE');
                        setLogs(prev => [{id: Date.now(), time: new Date().toLocaleTimeString(), dir: 'SYS', msg: translations[lang === 'zh' ? 'zh' : 'en'].log_tcp_offline}, ...prev].slice(0, 50));
                    } else if (status === 'FAILED') {
                        setTcpStatus('OFFLINE');
                        setLogs(prev => [{id: Date.now(), time: new Date().toLocaleTimeString(), dir: 'ERR', msg: translations[lang === 'zh' ? 'zh' : 'en'].log_tcp_failed}, ...prev].slice(0, 50));
    ...
    }, []); // 依赖空数组，只执行一次，所以内部对 lang 的引用可能是旧的，这是合理的，系统级连接日志不需要太频繁切换语言
            ...
            <header className="h-14 bg-slate-900/80 border-b border-cyan-500/30 flex items-center px-4 justify-between shrink-0 backdrop-blur-md relative z-10">
                ...
                <div className="flex items-center gap-6 text-xs font-bold">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-400">{t('ws_link')}</span>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-400">{t('tcp_link')}</span>
                    </div>
                </div>
            </header>
